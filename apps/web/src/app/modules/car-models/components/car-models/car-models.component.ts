...
export class CarModelsComponent implements OnInit {
  ...
  public deleteBrand(brand: BrandCarDto) {
    this.confirmDialogService.confirm({
      message: `Вы действительно хотите удалить марку "${brand.brand}"?<br /><br />ВНИМАНИЕ! У товаров удалятся все модели с такой маркой`,
      accept: () => {
        this.saving = true;
  ...
  public deleteModel(model: ModelCarDto, brand: BrandCarDto) {
    this.confirmDialogService.confirm({
      message: `Вы действительно хотите удалить модель "${model.model}" марки "${brand.brand}"?<br /><br />ВНИМАНИЕ! У товаров удалится модель`,
      accept: () => {
        this.saving = true;
